...
make_unix_socket(char *path)
{
    ...
    size_t maxlen = sizeof(addr.sun_path) - 1; // keep room for the null byte
    ...
    if (strlen(path) > maxlen) {
        warnx("truncating local socket path to: %.*s", (int)maxlen, path);
...
make_server_socket(char *host, char *port)
{
    ...
    if (host && strlen(host) > 5 && !strncmp(host, "unix:", 5)) {
